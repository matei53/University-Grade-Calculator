...
class CollapsibleYear(QWidget):
    ...
    def _toggle(self):
        """Switches visibility of the year content and updates the arrow icon."""
    ...
    def _create_mini_card(self, label, value):
        ...
        l.addWidget(val, alignment=Qt.AlignmentFlag.AlignCenter)
        return card
    ...
    def set_subjects(self, subjects, year_target_credits, passing_grade=5.0):
        ...
        :param year_target_credits: The credit requirement for this specific year.
        ...
        stats_row.addWidget(self._create_mini_card("Passed Credits", f"{total_passed_creds}/{year_target_credits}"))
        ...
        progress_percentage = int((total_passed_creds / year_target_credits) * 100) if year_target_credits > 0 else 0
        ...
        for sem in [1, 2]:
            ...
            if sem_subs:
                ...
                sorted_subs = sorted(sem_subs, key=lambda x: x['grade'] if x['grade'] is not None else -1, reverse=True)
                ...
                for sub in sorted_subs:
                    ...
                    c.setStyleSheet(f"color: {'#cc0000' if is_failed else '#888'}; font-size: 11px;")
                    ...
                    if grade_val is not None:
                        ...
                        g.setStyleSheet(f"font-weight: bold; color: {grade_color}; font-size: 14px;")
                    else:
                        # CASE: NOT GRADED (Smaller and subtle)
                        g.setText("Not Graded")
                        g.setStyleSheet("color: #999; font-size: 10px; font-weight: normal; font-style: italic;")
