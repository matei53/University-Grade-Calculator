...
from models import AcademicYear, Semester, Subject, Assessment, Grade
from schemas import SubjectResponse, AssessmentResponse
...
class SubjectService:
    ...
    def get_user_years(db: Session, user_id: int):
        years = db.query(AcademicYear).filter(
            AcademicYear.user_id == user_id
        ).order_by(AcademicYear.order_index).all()
        
        # Explicitly load all subjects and assessments to avoid lazy loading issues
        for year in years:
